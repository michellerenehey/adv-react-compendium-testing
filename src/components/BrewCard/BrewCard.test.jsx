...
//   render(<BrewCard breweries={[server]} />);

//   const brewName = await screen.findByRole('heading', { name: server[0].name });
//   expect(brewName).toBeInTheDocument();

//   const brewLocation = screen.getByText(`${server[0].city}, ${server[0].state}`);
//   expect(brewLocation).toBeInTheDocument();

//   const brewType = await screen.findByText(server[0].brewery_type);
